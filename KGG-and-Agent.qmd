...
```{python}
def generate_hermes_prompt(messages):
...
```{python}
user_input = "Alice loves Bob."
messages = generate_messages(user_input)
prompt = generate_hermes_prompt(messages)
assistant_response = generate_kg(prompt)
```
...
messages.append(
    {
        "role": "user",
        "content": "Correct (if needed) the generated knowledge graph and add the missing details.",
    }
)
```
